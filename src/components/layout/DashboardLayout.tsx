...
function Sidebar() {
  ...
  const currentUser = usePlatformStore((s) => s.users[0]);

  return (
    <aside
      className={cn(
        "fixed left-0 top-0 z-40 flex h-screen flex-col border-r border-white/[0.06] transition-all duration-300 ease-in-out",
        "bg-[#0a0a12]/80 backdrop-blur-2xl",
    ...
    >
      ...
      <div className="flex h-16 items-center gap-3 border-b border-white/[0.06] px-4">
        ...
        {!collapsed && (
          <div className="min-w-0 flex-1">
            <h1 className="truncate text-sm font-semibold text-white">{org.name}</h1>
            <p className="truncate text-[11px] text-white/40">Startup Ops Platform</p>
          </div>
        )}
      </div>
            ...
            >
              ...
              {!collapsed && (
                <div className="min-w-0 flex-1 text-left">
                  ...
                  {isActive && (
                    <p className="truncate text-[11px] font-normal text-white/30">{item.description}</p>
                  )}
                </div>
              )}
            </button>
      ...
      <div className="border-t border-white/[0.06] px-3 py-2">
        <button
          ...
          className="flex w-full items-center justify-center rounded-lg p-2 text-white/30 transition-colors hover:bg-white/[0.04] hover:text-white/60"
        >
      ...
      <div className="border-t border-white/[0.06] p-3">
        <div className={cn("flex items-center gap-3", collapsed && "justify-center")}>
          ...
          {!collapsed && currentUser && (
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium text-white/80">{currentUser.name}</p>
              <p className="truncate text-[11px] text-white/30">{currentUser.title}</p>
            </div>
          )}
          {!collapsed && (
            <button className="rounded-lg p-1.5 text-white/20 transition-colors hover:bg-white/[0.04] hover:text-white/50">
...
function Header() {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-30 flex h-16 items-center justify-between border-b border-white/[0.06] bg-[#07070d]/80 px-6 backdrop-blur-xl transition-all duration-300",
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.03] px-3 py-2">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="w-40 bg-transparent text-sm text-white/70 placeholder-white/20 outline-none"
          />
          <kbd className="rounded border border-white/[0.08] bg-white/[0.04] px-1.5 py-0.5 text-[10px] text-white/20">
            ⌘K
          </kbd>
        </div>

        {/* Theme Toggle */}
        <button className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-2.5 text-white/30 transition-colors hover:bg-white/[0.06] hover:text-white/60">
          <Moon className="h-4 w-4" />
        </button>

        {/* Notifications */}
        <button className="relative rounded-xl border border-white/[0.06] bg-white/[0.03] p-2.5 text-white/30 transition-colors hover:bg-white/[0.06] hover:text-white/60">
          <Bell className="h-4 w-4" />
          <span className="absolute -right-0.5 -top-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-gradient-to-r from-indigo-500 to-violet-500 text-[9px] font-bold text-white">
            3
          </span>
        </button>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
          <h3 className="text-sm font-semibold text-white/80">Execution Progress</h3>
          ...
          <div className="mt-4 grid grid-cols-4 gap-2">
            {[
              ...
              <div key={item.label} className="rounded-xl bg-white/[0.03] p-3 text-center">
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
          ...
          <p className="mt-1 text-xs text-white/30">Latest task updates across all projects</p>
            ...
            {tasks
              ...
              .map((task) => {
                ...
                return (
                  <div
                    key={task.id}
                    className="flex items-center gap-3 rounded-xl bg-white/[0.02] p-3 transition-colors hover:bg-white/[0.04]"
                  >
                    <div
                      className={cn(
                    ...
                    />
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm text-white/70">{task.title}</p>
...
function SettingsView() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
        <h3 className="text-sm font-semibold text-white/80">Organization</h3>
        <p className="mt-1 text-xs text-white/30">Manage your workspace settings</p>
        <div className="mt-4 space-y-3">
          <div className="flex items-center justify-between rounded-xl bg-white/[0.03] p-4">
            <div>
              <p className="text-sm font-medium text-white/70">Organization Name</p>
              <p className="text-xs text-white/30">{org.name}</p>
            </div>
          </div>
          <div className="flex items-center justify-between rounded-xl bg-white/[0.03] p-4">
            <div>
              <p className="text-sm font-medium text-white/70">Slug</p>
      ...
      <div className="rounded-2xl border border-red-500/20 bg-red-500/[0.03] p-6">
        ...
        <p className="mt-1 text-xs text-white/30">Reset all data to the original seed state</p>
        <button
          onClick={resetStore}
