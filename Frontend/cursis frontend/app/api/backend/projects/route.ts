...
export async function GET() {
  try { return NextResponse.json((await getFirestoreDb().collection("projects").get()).docs.map((doc) => withId(doc.id, doc.data()))); }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const doc = await getFirestoreDb().collection("projects").add({ ...body, status: body.status || "ACTIVE", color: body.color || "#6366f1", createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
