...
export async function GET(request: NextRequest) {
  try {
    ...
    const tasks = (await firestore.collection("tasks").get()).docs.map((doc) => withId(doc.id, doc.data())).filter((task) => !searchParams.get("projectId") || task.projectId === searchParams.get("projectId")).filter((task) => !searchParams.get("status") || task.status === searchParams.get("status"));
...
export async function POST(request: NextRequest) {
  try {
    ...
    const doc = await firestore.collection("tasks").add(payload);
