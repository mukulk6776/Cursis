...
export function AuthInput({ id, label, type = "text", placeholder, value, onChange, autoComplete, error, showPasswordToggle, passwordVisible, onPasswordVisibilityChange }: AuthInputProps) {
  ...
  return (
    <div>
      ...
      <div className="relative">
        <input id={id} name={id} type={inputType} placeholder={placeholder} value={value} onChange={onChange} autoComplete={autoComplete} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} className={`h-11 w-full rounded-lg border bg-white px-3 text-sm text-slate-900 outline-none transition-colors placeholder:text-slate-400 focus:border-slate-500 ${error ? "border-rose-400" : "border-slate-200"} ${showPasswordToggle ? "pr-11" : ""}`} />
