...
export default function SignupPage() {
  ...
  const saveTokenAndRedirect = (token: string, targetUrl: string = '/dashboard') => {
    if (token) {
      try {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      saveTokenAndRedirect(token, '/dashboard');
    } catch (err: any) {
      setErrorMsg(err.message || 'Failed to create workspace account.');
      setLoading(false);
    }
  };
    ...
    >
      <div style={{ marginBottom: 'var(--sp-6, 24px)', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '13px', color: 'var(--text-secondary, #64748b)', marginTop: '6px' }}>
          Create your autonomous AI execution workspace in seconds
        </p>
      </div>
      ...
      >
        ...
        <div style={{ marginTop: '20px', textAlign: 'center', fontSize: '13px', color: '#64748b' }}>
          ...
          <Link href="/login" style={{ color: '#ff5710', fontWeight: 700, textDecoration: 'none' }}>
            Sign in
          </Link>
        </div>
      </div>
